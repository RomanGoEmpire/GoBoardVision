...
MAX_SIZES = int(WINDOW_SIZE / 20)
HALF_MAX_SIZE = int(MAX_SIZES / 2)
...
# black = 1, white = 2
initial_board = np.zeros((ROWS, COLS), dtype='int16')
...
def calculate_positions():
    all_sizes = np.zeros((MAX_SIZES + 1, ROWS, COLS, 2), dtype='int16')
    h, w = WINDOW_SIZE, WINDOW_SIZE
    dy, dx = int((h - (h / 20)) / ROWS), int((w - (w / 20)) / COLS)
    for i in range(MAX_SIZES + 1):
...
def create_all_slider():
    ...
    cv2.createTrackbar('size', 'Grid Image', size, MAX_SIZES, on_threshold)
...
def color_identifed_grid_greyscale(img, isBlack):
    ...
    for row in range(ROWS):
        for col in range(COLS):
            x = ALL_GRID_POINTS[size][row][col][0]
            y = ALL_GRID_POINTS[size][row][col][1]
            max_x = ALL_GRID_POINTS[MAX_SIZES][row][col][0]
            max_y = ALL_GRID_POINTS[MAX_SIZES][row][col][1]
            ...
            if size != 0:
                ...
                if color == 255:
                    board[row][col] = 1 if isBlack else 2

            cv2.rectangle(new_img, (max_x, max_y), (max_x + MAX_SIZES, max_y + MAX_SIZES), color=color,
                          thickness=-1)
            cv2.rectangle(new_img, (max_x, max_y), (max_x + MAX_SIZES, max_y + MAX_SIZES), color=127, thickness=1)
    return new_img
...
def is_valid_board():
    ...
    print(np.array_equal(board, next_board), frame_counter, is_blacks_turn, count_black(), black_stones, count_white(),
          white_stones)
...
def update_board():
    global black_stones, white_stones, is_blacks_turn
    if is_blacks_turn:
        black_stones += 1
        ...
        white_stones += 1
        is_blacks_turn = True
    print(is_blacks_turn, white_stones, black_stones)
